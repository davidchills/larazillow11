<template>
	...
	<Box v-if="listing.images.length" class="mt-4">
		...
		<section class="mt-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-4">
			<div v-for="image in listing.images" :key="image.id"
				class="flex flex-col justify-between">
				<img :src="image.src" class="w-full h-48 object-cover rounded-md" />
				<Link :href="route('realtor.listing.image.destroy', { listing: props.listing.id, image: image.id })"
					method="delete"
					as="button"
					class="btn-outline text-xs font-medium text-center mt-2">
					Delete
				</Link>
